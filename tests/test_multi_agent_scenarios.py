...
SCENARIOS = [
    ...
    # 수행 — 조회 (2개) — executor 호출 + 목록 조회
    (3, "조조형우 상품페이지 중에 '월간 투자' 어쩌고 하는 상품페이지 있지?", "executor", ["월간 투자", "조조형우"]),
    (25, "노출중인 조조형우 상품페이지 뭐가 있어? 비공개하면 다른거 공개되는거 있나?", "executor", ["조조형우", "페이지"]),
...
def main():
    print("=" * 60)
    print(f"멀티 에이전트 시나리오 테스트 ({len(SCENARIOS)}개)")
    print("=" * 60)
