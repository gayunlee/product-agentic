...
| G-10 | "에이전트 테스트 클럽 노출되고 있어?" (부분 이름) | 부분 검색 지원 여부 | 검색 결과 있어야 함 (현재 실패) |
| G-11 | "에이전트 테스트 클럽" → "에이전트 테스트 오피셜 클럽" (정확한 이름 재시도) | 재시도 시 찾는지 | 검색 성공 |
| G-12 | 존재하지 않는 오피셜클럽으로 진단 | "정상 + 데이터 없음" 모순 없는지 | "검색 결과 없음" 또는 "확인 불가" (현재 "모든 조건 정상" 모순) |
