...
### 최우선 규칙: "수정/변경/편집/바꾸" 단어가 있으면 무조건 edit
- "수정할래" → edit
- "변경하려고" → edit
- "옵션 바꿀래" → edit
- "페이지 설정 수정" → edit

### diagnose는 "노출 문제"에만 사용
- "안 보여/왜 안 나와" → diagnose
- "수정/변경/편집" → diagnose 절대 아님, edit임

### 나머지
- "만들래/세팅해줘" → setup
- "추가/등록해줘" → create_option
- "뭐있어?/목록" → list_pages 또는 list_options
- "비공개/공개해줘" → toggle_display 또는 toggle_page
...
def classify_node(state: AgentState) -> dict:
    ...
    # ── 최소 가드레일: LLM이 자주 틀리는 패턴 보정 ──
    # "수정/변경/편집" → edit (Haiku가 diagnose로 잘못 분류하는 것 방지)
    if any(kw in last_msg for kw in ["수정", "변경", "편집", "바꿀", "바꾸"]):
        # LLM 대신 직접 edit로 분류하되, master_name은 collected에서 가져옴
        master_name = collected.get("master_name", collected.get("_requested_master", ""))
        target = "option" if any(kw in last_msg for kw in ["옵션", "상품옵션"]) else "page"
        print(f"🧠 가드레일 분류: edit (수정 키워드 감지) master={master_name} target={target}")
        return {
            "action": "edit",
            "collected": {**collected, "_edit_target": target},
            "response_message": "",
        }
    ...
    # 대화 히스토리 — 어시스턴트 응답은 축약, 유저 메시지는 원문 유지
    recent = []
    history_msgs = [m for m in messages if isinstance(m, (HumanMessage, AIMessage))]
    for m in history_msgs[-6:]:
        if isinstance(m, HumanMessage):
            recent.append(HumanMessage(content=m.content[:300]))
        else:
            # 어시스턴트 응답은 첫 줄만 (긴 진단 결과가 맥락을 오염시키는 것 방지)
            text = m.content if isinstance(m.content, str) else str(m.content)
            first_line = text.split("\n")[0][:100]
            recent.append(AIMessage(content=first_line))
...
def edit_node(state: AgentState) -> dict:
    ...
    _track_phase("edit")
    collected = state.get("collected", {})
    print(f"📝 [edit] collected keys: {list(collected.keys())}, product_type={collected.get('product_type','')}")
    ...
    if not page_id and master_id:
        ...
        if isinstance(pages, list) and len(pages) > 0:
            ...
            product_type = collected.get("product_type", "")
            matched = None
            # 1. page_code로 매칭
            if page_code:
                matched = next((p for p in pages if str(p.get("code", "")) == str(page_code)), None)
            # 2. product_type에서 페이지명 힌트 추출 (LLM이 "단건상품"을 product_type에 넣는 경우)
            if not matched and product_type:
                matched = next((p for p in pages if product_type in p.get("title", "")), None)
            # 3. LLM 메시지에서 페이지명 매칭
            if not matched and llm_message:
                for p in pages:
                    title = p.get("title", "")
                    if title and title in llm_message:
                        matched = p
                        break
            # 4. 유저 메시지에서 페이지명 매칭
            if not matched:
                ...
                if last_user_msg:
                    for p in pages:
                        title = p.get("title", "")
                        if title and title in last_user_msg:
                            matched = p
                            break
            # 5. 페이지 1개면 자동 선택
