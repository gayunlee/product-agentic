...
⚠️ 액션 선택 가이드 (정확히 따르세요):
- "상품 만들래/세팅해줘" → setup
- "옵션 추가/등록해줘" → create_option
- **"수정/변경/편집/바꾸려고"가 포함되면 → edit** (target: option 또는 page)
- "마스터 있어?/검색해줘" → search_master
- "뭐있어?/목록/조회" → list_pages 또는 list_options
- "비공개해줘/공개해줘" → toggle_display 또는 toggle_page
- "안 보여/왜 안 나와/노출" → diagnose (노출 문제 진단에만 사용)
- diagnose는 "노출 문제 진단"에만 사용. 수정/변경 요청에는 절대 사용 금지.
