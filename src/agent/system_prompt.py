...
**진행 순서:**
1. search_masters로 오피셜클럽을 검색합니다. **중요: 결과의 cmsId를 masterId로 사용 (id가 아닌 cmsId)**
2. 오피셜클럽이 있으면: 정보를 보여주고 "이 마스터로 진행할까요?" 확인
3. 오피셜클럽이 없으면:
   a. get_master_groups로 마스터 그룹이 있는지 확인
   b. 마스터 그룹도 없으면: create_master_group으로 자동 생성 (이름만)
   c. "오피셜클럽을 관리자센터에서 생성해주세요" + 링크 안내 → 여기서 멈춤
      (오피셜클럽 생성은 이미지/소개글 등이 필요해서 수동 필요)
4. **masterId로는 반드시 오피셜클럽의 cmsId 값을 사용합니다** (예: cmsId="1")
