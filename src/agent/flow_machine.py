...
- 오피셜클럽 생성 시 어떤 마스터 그룹에 속하는지 선택 필요 (없으면 먼저 생성)
...
- **상품 페이지 노출 규칙**: 공개 상태(ACTIVE) ≠ 실제 노출.
  - 기간공개 페이지 중 오늘 날짜가 기간 내인 것 → 최우선 노출
  - 없으면 → 상시공개(isAlwaysPublic) 페이지가 노출 (마스터당 1개만 가능)
  - "공개 중인 페이지 5개"와 "실제 노출 중인 페이지 1개"는 다른 개념
...
def _find_active_page(pages: list[dict]) -> dict | None:
    ...
    1. 기간공개 중 오늘이 기간 내인 것 (최우선)
    2. 없으면 상시공개(isAlwaysPublic) 페이지
    """
...
class FlowMachine:
    ...
    def handle(self, message: str, context: dict | None = None) -> Response:
        ...
        try:
            ...
            # ── Phase 1.5: 가드레일 (LLM이 분류한 뒤 보정) ──
            has_edit_keyword = any(kw in msg for kw in ["수정", "변경", "편집", "바꿀", "바꾸"])
            if has_edit_keyword and understood.get("action") not in ("edit", "direct"):
                target = "option" if any(kw in msg for kw in ["옵션", "상품옵션"]) else "page"
                print(f"📍 가드레일: '{understood.get('action')}' → edit 교정 (수정 키워드, target={target})")
                understood["action"] = "edit"
                understood.setdefault("params", {})["target"] = target
    ...
    def _route_action(self, action: str, params: dict, llm_message: str) -> Response:
        ...
        if action == "direct":
            ...
            if direct_target == "toggle_display_exec":
                return self._exec_toggle_display(direct_params)
            if direct_target == "toggle_page_exec":
                return self._exec_toggle_page(direct_params)
            # 일반 라우팅
        ...
        # ── 상태 변경 액션 (확인 → 실행 → 결과 확인 패턴) ──
        if action == "toggle_display":
            return self._action_toggle_display(params, llm_message)
        if action == "toggle_page":
            return self._action_toggle_page(params, llm_message)
    ...
    def _action_list_pages(self, params: dict) -> Response:
        ...
        # 실제 노출 중인 페이지 판별
        all_active = [p for p in _api_get("/v1/product-group", {"masterId": cms_id}) or [] if p.get("status") == "ACTIVE"]
    ...
    # ══════════════════════════════════════
    # 상태 변경 (확인 → 실행 → 결과 확인 패턴)
    # ══════════════════════════════════════

    def _action_toggle_display(self, params: dict, llm_message: str) -> Response:
        """상품 옵션 공개/비공개 — Step 1: 확인."""
        option_name = params.get("option_name", "")
        action_type = params.get("action", "hide")  # show/hide
        page_id = self.data.get("product_page_id", "")
        cms_id = self.data.get("master_cms_id", "")
        label = "공개" if action_type == "show" else "비공개"
        ...
        if not isinstance(products, list):
            return Response(message="상품 옵션을 조회할 수 없습니다.")

        target = None
        for p in products:
            if option_name and option_name in p.get("name", ""):
                target = p
                break
        if not target:
            # 이름 매칭 실패 → 목록 보여주기
    ...
    def _exec_toggle_display(self, params: dict) -> Response:
        """상품 옵션 공개/비공개 — Step 2: 실행."""
        product_id = params.get("product_id", "")
        is_display = params.get("is_display", "false") == "true"
        label = "공개" if is_display else "비공개"
        page_id = self.data.get("product_page_id", "")
        cms_id = self.data.get("master_cms_id", "")
        ...
        return Response(
            ...
            buttons=[
                {"type": "navigate", "label": "📦 페이지에서 확인", "url": f"/product/page/{page_id}?masterId={cms_id}&tab=options", "variant": "primary", "description": "변경 결과를 확인합니다."},
            ],
        )

    def _action_toggle_page(self, params: dict, llm_message: str) -> Response:
        """상품 페이지 공개/비공개 — Step 1: 확인."""
        page_code = params.get("page_code", "")
        action_type = params.get("action", "hide")
        cms_id = self.data.get("master_cms_id", "")
        label = "공개" if action_type == "show" else "비공개"
        ...
        target = None
        for p in pages:
            if page_code and (str(p.get("code", "")) == str(page_code) or page_code in p.get("title", "")):
                target = p
                break
        if not target:
            rows = "\n".join(f"- {p.get('title', '')} (코드: {p.get('code', '')}, {_status_label(p.get('status', ''))})" for p in pages)
            return Response(message=f"'{page_code}' 페이지를 찾을 수 없습니다.\n\n상품 페이지:\n{rows}\n\n코드 번호나 페이지명을 알려주세요.")

        page_id = target.get("id", "")
        current = _status_label(target.get("status", ""))

        return Response(
            message=f"**{target.get('title', '')}** (코드: {target.get('code', '')})를 {label} 처리할까요?\n\n현재 상태: {current}\n\n아래에서 직접 변경도 가능합니다.",
            buttons=[
                {"type": "action", "label": f"⚡ {label} 처리", "action": {"type": "direct", "target": "toggle_page_exec", "params": {"page_id": page_id, "status": "ACTIVE" if action_type == "show" else "INACTIVE"}}, "variant": "primary", "description": f"에이전트가 {label} 처리합니다."},
                {"type": "navigate", "label": "📄 직접 변경하러 가기", "url": f"/product/page/{page_id}?masterId={cms_id}&tab=settings", "variant": "secondary", "description": "상품 페이지 설정에서 직접 변경합니다."},
            ],
        )

    def _exec_toggle_page(self, params: dict) -> Response:
        """상품 페이지 공개/비공개 — Step 2: 실행."""
        page_id = params.get("page_id", "")
        status = params.get("status", "INACTIVE")
        label = _status_label(status)
        cms_id = self.data.get("master_cms_id", "")

        result = _api_patch("/v1/product-group/status", {"id": page_id, "status": status})
        if isinstance(result, dict) and result.get("error"):
            return Response(message=f"⚠️ {label} 처리 실패: {result.get('guide', '')}")

        return Response(
            message=f"✅ {label} 처리 완료!",
            buttons=[
                {"type": "navigate", "label": "📄 페이지에서 확인", "url": f"/product/page/{page_id}?masterId={cms_id}&tab=settings", "variant": "primary", "description": "변경 결과를 확인합니다."},
            ],
        )
    ...
    def _exec_check_master(self) -> Response:
        ...
        self.state = "wait_create_master"
        return Response(message=msg, buttons=buttons, step=_step_meta("check_master"))
    ...
    def _handle_wait(self, msg: str) -> Response:
        ...
        if self.state == "wait_create_master":
            if "완료" in msg:
                # 다시 마스터 검색
                return self._exec_check_master()
            return Response(message=msg, need_llm=True, mode="guide", step=_step_meta("check_master"))
    ...
    def _exec_diagnose(self, msg: str) -> Response:
        ...
        if page_active and active_pages:
            showing = showing if 'showing' in dir() else active_pages[0]
            p = showing or active_pages[0]
            if p.get("isAlwaysPublic"):
